...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'fli':
				dialogue = CoolUtil.coolTextFile(Paths.txt('fli/fliDialogue'));
		}
		...
		else if (SONG.song.toLowerCase() == 'fli')
		{
			defaultCamZoom = 0.9;
			curStage = 'void';
			var bg:FlxSprite = new FlxSprite(-300, -200).makeGraphic(Std.int(FlxG.width * 1.7), Std.int(FlxG.height * 1.8), 0xFFFFFFFF);
			bg.active = false;
			add(bg);
		}
		else
	...
	override public function update(elapsed:Float)
	{
		...
		#else
		if (FlxG.keys.justPressed.EIGHT)
			FlxG.switchState(new WrongEngineLmao());
		#end
